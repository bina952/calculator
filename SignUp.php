   ...
   if($passwordConfirm ==''){
        ...
        if($password != $passwordConfirm){
         ...
         if(strlen($password)>20){ // Max 
          ...
          $sql="select * from project where (username='$username' or email='$email');";
...
$row = mysqli_fetch_assoc($res);
    ...
    $password = password_hash($password,PASSWORD_BCRYPT,$options);
            ...
            $result = mysqli_query($conn,"INSERT into project(fname,lname,username,email,password,date) values('$fname','$lname','$username','$email','$password','$date')");
